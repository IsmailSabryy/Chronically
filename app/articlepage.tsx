...
const formatToUTCA = (isoDate: string) => {
  ...
  const fetchComments = async () => {
    const response = await fetch('http://localhost:3000/get_comments_article', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  const postComment = async (comment : string) => {
    ...
    const response = await fetch('http://localhost:3000/comment_article', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleShare = async () => {
    try {
      ...
      const data = await response.json();
      if (data.username) {
        await fetch('http://localhost:3000/share_articles', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
